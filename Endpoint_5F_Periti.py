...
# ── POST rimborso con integrazione preventivo ──────────────────────────────────

@app.route('/sinistro/<id_sinistro>/perito/<id_perito>/pratica/<id_perizia>/rimborso', methods=['POST'])
def registra_rimborso_con_preventivo(id_sinistro, id_perito, id_perizia):
    data = request.get_json()
    if not data:
    ...
    # Costruiamo l'oggetto preventivo da inserire nella pratica
    # Se il frontend te lo passa già completo, puoi prenderlo direttamente
    preventivo_doc = {
        "officina_id":     data.get("officina_id"),
        "data_preventivo": data.get("data_preventivo"),
        "importo_totale":  data.get("importo_totale"),
        "dettagli":        data.get("dettagli"),
        "accettato":       data.get("accettato", True),
        "attivo":          data.get("attivo", True)
    }

    # Aggiorniamo la collezione 'Pratica' (o col_perizie in base a dove salvi)
    res = col_pratiche.update_one(
        {"_id": p_id},
        {"$set": {
            "stima_danno":        data.get("importo_totale"), # Sincronizziamo la stima col preventivo
            "preventivo":         preventivo_doc,             # <--- LA NUOVA VOCE
    ...
    # Aggiorniamo anche lo stato del sinistro per coerenza
    try:
        col_sinistri.update_one(
            {"_id": ObjectId(id_sinistro)},
            {"$set": {
        ...
        print(f"Errore aggiornamento sinistro: {e}")

    return jsonify({
        "status": "Rimborso e preventivo salvati",
        "preventivo_inserito": preventivo_doc
    }), 200
